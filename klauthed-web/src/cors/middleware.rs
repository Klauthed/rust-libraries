...
use actix_web::http::header::{
    ...
    ACCESS_CONTROL_ALLOW_ORIGIN, ACCESS_CONTROL_EXPOSE_HEADERS, ACCESS_CONTROL_MAX_AGE, HeaderMap,
...
impl DynamicCors {
    ...
    pub fn new(config: CorsConfig, registry: Arc<dyn CorsOriginRegistry>) -> Self {
        ...
        Self { static_origins: Arc::new(static_set), registry, config: Arc::new(config) }
    }

    /// Convenience: use an [`InMemoryOriginRegistry`] (tenant origins added
    /// at runtime via `registry.insert()`).
    pub fn with_memory_registry(config: CorsConfig) -> (Self, Arc<InMemoryOriginRegistry>) {
    ...
    B: actix_web::body::MessageBody + 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let origin: Option<String> =
            req.headers().get(ORIGIN).and_then(|v| v.to_str().ok()).map(str::to_owned);
        ...
        Box::pin(async move {
            ...
            let allowed_origin: Option<String> = match origin {
                ...
                Some(ref o) => {
                    let allowed =
                        static_origins.contains(o.as_str()) || registry.is_allowed(o).await;
            ...
            if *req.method() == Method::OPTIONS {
                let mut resp = HttpResponse::NoContent().finish().map_into_boxed_body();
            ...
            let mut res = service.call(req).await.map(ServiceResponse::map_into_boxed_body)?;
...
mod tests {
    ...
    async fn static_platform_origin_is_allowed_without_registry() {
        ...
        let app =
            http_test::init_service(App::new().wrap(cors).route("/", web::get().to(ok_handler)))
                .await;
    ...
    async fn tenant_origin_from_registry_is_allowed() {
        ...
        let cors =
            DynamicCors::new(platform_config(), Arc::clone(&reg) as Arc<dyn CorsOriginRegistry>);
        let app =
            http_test::init_service(App::new().wrap(cors).route("/", web::get().to(ok_handler)))
                .await;
    ...
    async fn options_preflight_for_allowed_origin_returns_204() {
        ...
        let cors = DynamicCors::new(platform_config(), reg);
        let app =
            http_test::init_service(App::new().wrap(cors).route("/api", web::get().to(ok_handler)))
                .await;
