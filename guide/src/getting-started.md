...
the framework's `/health` and `/health/ready` probes. Optional flags add more
wiring, and compose:

- `--with-jwt` — a `/login` endpoint and a JWT-protected `/api/me` route.
- `--database postgres|mysql|sqlite` — a connection pool wired into the web layer
  and its readiness probe.
- `--with-scheduler` — an interval scheduler running an example recurring task.

Grow it further by enabling more `klauthed` features (below). The rest of this
page shows what that wiring looks like by hand.
